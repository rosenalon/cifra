...
function time(){
	...
	dref.setHours(9,30,0,0);
	var order = ["giulio_b",	"cengiz",	"coffee2",	"matthieu",	"florent", 	"lunch",	"zohar",	"song",	"coffee3",	"yue", "sueyeon",		"dinner"];
	var times = [9.5, 		10.25, 		11, 		11.5, 	    12.25, 		13, 		14.5, 		15.25,		15.75, 		16.5, 	 17.25,	     	20];
	if(order.length != times.length){
		alert("order length ("+order.length+") != times length ("+times.length+")");
	}
	if(d.getHours() >= 9.5 && d.getHours() < 22){
		...
		if(hrs < 18 - 9.5){
			if(hrs >= 17.25 - 9.5){
				...
				y_pos = y_top + (hrs + 9.5 - 16.5) / 0.75 * dy; 
			} else {
				...
				var y_top = elem.getBoundingClientRect().top - document.body.getBoundingClientRect().top,
					dy = elem.getBoundingClientRect().bottom - y_top  - document.body.getBoundingClientRect().top;
				y_pos = y_top + (hrs + 9.5 - times[i_-1]) / (times[i_] - times[i_-1]) * dy;
			}
		} else if(hrs < (20 - 9.5)) {
			...
			y_pos = y_top + (y_bottom - y_top) * ((hrs - (17.25 - 9.5)) / (20-17.25));
		} else  {
			var elem = document.getElementById("td_dinner");
			var y_top = 5+elem.getBoundingClientRect().top - document.body.getBoundingClientRect().top,
				dy = elem.getBoundingClientRect().bottom - y_top  - document.body.getBoundingClientRect().top;
			y_pos = y_top + (hrs + 9.5 - 20) / (22 - 20) * dy;
		}
